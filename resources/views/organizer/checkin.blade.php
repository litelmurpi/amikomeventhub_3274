...
@section('content')
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
            <div class="flex items-center gap-2">
                <span class="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-lg text-xs font-bold uppercase tracking-wider">Tenant Isolation On</span>
                <span class="text-xs text-slate-400 font-semibold">{{ $org->name }}</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-black text-slate-900 mt-1">Check-in Tiket Peserta</h1>
            <p class="text-slate-500 font-medium text-sm sm:text-base">Pindai QR Code E-Ticket peserta khusus untuk event {{ $org->name }}.</p>
        </div>

        <!-- Event Filter Dropdown -->
        @if($myEvents->count() > 0)
        <form method="GET" action="{{ route('organizer.checkin') }}" class="w-full md:w-auto">
            <select name="event_id" onchange="this.form.submit()"
                class="w-full md:w-64 px-4 py-3 rounded-xl border-slate-200 border bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition font-bold text-sm text-slate-700">
                <option value="">Semua Event Saya ({{ $myEvents->count() }})</option>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 bg-white rounded-3xl sm:rounded-[2.5rem] border border-slate-100 p-6 sm:p-8 shadow-sm h-fit">
            ...
            <p class="text-slate-400 text-xs font-semibold mb-6">Gunakan kamera HP/Webcam untuk scan QR E-Ticket peserta di pintu masuk.</p>
            ...
            <!-- Form Manual -->
            <form action="{{ route('organizer.checkin.verify') }}" method="POST" class="space-y-6">
                @csrf
                <div>
            ...
            <div class="overflow-x-auto min-w-full">
                <table class="w-full text-left border-collapse min-w-[500px]">
                    ...
                    <tbody id="recent-checkins-tbody" class="divide-y border-t">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-slate-400 text-sm">Belum ada check-in untuk event Anda.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
<script>
    ...
    function onScanSuccess(decodedText, decodedResult) {
        ...
        fetch("{{ route('organizer.checkin.ajax') }}", {
            method: "POST",
            headers: {
