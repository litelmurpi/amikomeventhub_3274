...
@section('content')
    ...
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-10">
        ...
        <div class="flex items-center gap-4">
            <div class="text-right hidden md:block">
                ...
                <p class="text-xs text-slate-400">Pemilik Organisasi</p>
            </div>
            <div class="w-12 h-12 bg-white rounded-2xl shadow-sm border flex items-center justify-center p-1">
                <img src="https://ui-avatars.com/api/?name={{ urlencode($org->name) }}&background=6366f1&color=fff"
                    class="rounded-xl">
            </div>
        </div>
    </header>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <p class="text-slate-400 text-sm font-bold uppercase mb-1">Event Saya</p>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 border-b flex justify-between items-center">
            ...
            <a href="{{ route('organizer.events') }}" class="text-indigo-600 font-bold hover:underline">Kelola Event</a>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y border-t">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-8 py-6">
                            <span class="px-3 py-1 bg-{{ $trx->status == 'Success' ? 'green' : ($trx->status == 'Pending' ? 'orange' : 'slate') }}-100 text-{{ $trx->status == 'Success' ? 'green' : ($trx->status == 'Pending' ? 'orange' : 'slate') }}-700 rounded-lg text-xs font-bold uppercase">{{ $trx->status }}</span>
                        </td>
                        <td class="px-8 py-6 font-black text-indigo-600">Rp {{ number_format($trx->total_price, 0, ',', '.') }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="4" class="px-8 py-10 text-center text-slate-400 text-sm">Belum ada transaksi untuk event Anda.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
