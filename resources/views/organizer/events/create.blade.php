...
@section('title', 'Buat Event Baru - Amikom Event Hub')

@section('content')
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-10">
        <div>
            <h1 class="text-2xl sm:text-3xl font-black">Buat Event Baru</h1>
            <p class="text-slate-500 text-sm sm:text-base font-medium">Lengkapi form di bawah untuk memublikasikan event {{ $org->name }}.</p>
        </div>
        ...
        <form action="{{ route('organizer.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', 0) }}" required placeholder="0 untuk gratis" min="0" 
                        class="w-full px-5 py-3 rounded-xl border-slate-200 border bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition">
                    <p class="text-xs text-slate-400 mt-1">Isi 0 jika event ini GRATIS.</p>
                ...
                <div>
                    ...
                    <input type="number" name="stock" value="{{ old('stock', 100) }}" required placeholder="Jumlah tiket" min="1" 
                        class="w-full px-5 py-3 rounded-xl border-slate-200 border bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition">
