...
@section('content')
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-10">
        <div>
            <h1 class="text-2xl sm:text-3xl font-black">Edit Event</h1>
            <p class="text-slate-500 text-sm sm:text-base font-medium">Perbarui informasi event {{ $event->title }}.</p>
        </div>
        <a href="{{ route('organizer.events') }}" class="px-6 py-3 bg-slate-100 text-slate-600 rounded-2xl font-bold hover:bg-slate-200 transition text-center">
            Kembali
        </a>
    </header>
        ...
        <form action="{{ route('organizer.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date" value="{{ old('date', \Carbon\Carbon::parse($event->getRawOriginal('date'))->format('Y-m-d\TH:i')) }}" required 
                        class="w-full px-5 py-3 rounded-xl border-slate-200 border bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition">
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $event->price_value) }}" required placeholder="0 untuk gratis" min="0" 
                        class="w-full px-5 py-3 rounded-xl border-slate-200 border bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition">
                ...
                <div>
                    ...
                    <input type="number" name="stock" value="{{ old('stock', $event->stock) }}" required placeholder="Jumlah tiket" min="0" 
                        class="w-full px-5 py-3 rounded-xl border-slate-200 border bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition">
            ...
            <!-- Description -->
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Deskripsi Utama</label>
            ...
            <div>
                ...
                @if($event->poster_path)
                    <div class="mb-3">
                        <img src="{{ asset($event->poster_path) }}" alt="Poster" class="w-24 h-32 rounded-xl object-cover border shadow-sm">
                    </div>
                @endif
