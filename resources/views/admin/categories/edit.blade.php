...
@section('title', 'Edit Kategori - Admin Amikom Event Hub')

@section('content')
    <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-10">
        <div>
            <h1 class="text-2xl sm:text-3xl font-black">Edit Kategori</h1>
            <p class="text-slate-500 text-sm sm:text-base font-medium">Ubah nama kategori yang sudah ada.</p>
        </div>
        <a href="{{ route('admin.categories') }}" class="px-6 py-3 bg-slate-100 text-slate-600 rounded-2xl font-bold hover:bg-slate-200 transition text-center">
            Kembali
        </a>
    </header>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-8">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" required placeholder="Masukkan nama kategori" 
            ...
            <div>
                ...
                <div class="flex gap-2 w-full md:w-1/2">
                    <input type="text" id="slug" name="slug" value="{{ old('slug', $category->slug) }}" required placeholder="kategori-slug" 
                ...
                <p class="text-slate-400 text-xs mt-1.5">Untuk menjaga keutuhan URL, slug terkunci secara default. Klik tombol gembok untuk mengubah manual jika diperlukan.</p>
