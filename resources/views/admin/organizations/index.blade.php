...
<main class="flex-1 p-10 overflow-y-auto">
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-8 py-6 bg-slate-50/50 border-b">
            <form method="GET" action="{{ route('admin.organizations') }}" id="search-form" class="flex gap-2">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.organizations') }}" class="px-4 py-3 bg-slate-200 text-slate-700 rounded-xl font-bold hover:bg-slate-300 transition flex items-center">Reset</a>
                @endif
            </form>
        </div>
                ...
                <tbody class="divide-y border-t">
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-8 py-6 text-right space-x-2">
                            ...
                            @if(!$org->isRejected())
                                <button type="button" onclick="openRejectModal({{ $org->id }}, '{{ addslashes($org->name) }}')" class="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-lg text-xs font-bold transition">
                                    Tolak / Nonaktifkan
                                </button>
                            @endif
                        </td>
    ...
    <div id="rejectModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm hidden items-center justify-center z-50 p-4">
        <div class="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl border border-slate-100">
            <h3 class="text-xl font-black text-slate-800 mb-2">Tolak Pendaftaran Organisasi</h3>
            <p class="text-sm text-slate-500 mb-6" id="rejectModalOrgName">Berikan alasan penolakan pendaftaran organisasi ini:</p>
            
            <form id="rejectForm" method="POST" action="">
                @csrf
                <div class="mb-6">
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Alasan Penolakan</label>
                    <textarea name="rejection_reason" rows="4" required class="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-800 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none" placeholder="Contoh: Nama/Logo organisasi tidak sesuai dengan standar Amikom."></textarea>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl text-xs shadow-lg shadow-rose-600/30 transition">
