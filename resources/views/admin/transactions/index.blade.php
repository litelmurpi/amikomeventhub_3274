    ...
    <main class="flex-1 p-10 overflow-y-auto">
        <header class="flex justify-between items-center mb-10">
            ...
            <div class="flex gap-3 items-center">
                <form action="{{ route('admin.transactions.expire-pending') }}" method="POST"
                    onsubmit="return confirm('Apakah Anda yakin ingin mengubah semua transaksi pending yang berusia > 24 jam menjadi expired?')">
                    @csrf
                    <button type="submit"
                        class="px-5 py-3 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-600 rounded-2xl font-bold text-sm transition flex items-center gap-1.5 shadow-sm">
                        🧹 Bersihkan Kadaluarsa
                    </button>
                </form>
                <button
                    class="px-5 py-3 border-2 border-slate-200 rounded-2xl font-bold text-sm hover:bg-white hover:border-indigo-600 hover:text-indigo-600 transition">
                    Ekspor Excel
                </button>
                <button
                    class="px-5 py-3 bg-indigo-600 text-white rounded-2xl font-bold text-sm shadow-lg hover:bg-indigo-700 transition">
                    Unduh PDF
                </button>
            </div>
        </header>
        ...
        @if(session('success'))
            <div class="mb-6 p-4 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-2xl text-sm font-bold shadow-sm">
                {{ session('success') }}
            </div>
        @endif
            ...
            <form action="{{ route('admin.transactions') }}" method="GET" class="px-8 py-6 bg-slate-50/50 border-b flex flex-wrap gap-4 items-center">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request('search') || request('status'))
                        <a href="{{ route('admin.transactions') }}" class="px-5 py-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 outline-none text-sm font-bold text-slate-500 transition flex items-center">
                            Reset
                        </a>
                    @endif
                </div>
