...
<header class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6 md:mb-10">
    <div>
        <h1 class="text-2xl md:text-3xl font-black text-slate-800">Kelola Partner</h1>
        <p class="text-sm md:text-base text-slate-500 font-medium">Atur partner & sponsor untuk event-event Amikom.</p>
    </div>
...
<div class="bg-white rounded-2xl md:rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
    <div class="p-4 md:px-8 md:py-6 bg-slate-50/50 border-b">
        <form method="GET" action="{{ route('admin.partners') }}" id="search-form" class="flex flex-col sm:flex-row gap-2">
            ...
            <div class="flex gap-2">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.partners') }}" class="px-4 py-2.5 md:py-3 bg-slate-200 text-slate-700 rounded-xl font-bold text-sm hover:bg-slate-300 transition flex items-center justify-center">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        <table class="w-full text-left border-collapse min-w-[550px]">
            <thead class="bg-slate-50 text-slate-400 uppercase text-[10px] font-black tracking-widest">
                <tr>
                    ...
                    <th class="px-6 py-4 w-20">Logo</th>
                    <th class="px-6 py-4">Nama Partner</th>
                    <th class="px-6 py-4">URL Logo</th>
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4">
                        <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl border border-slate-100 bg-white p-1 flex items-center justify-center overflow-hidden shadow-sm">
                    ...
                    <td class="px-6 py-4">
                        <p class="font-black text-slate-800 text-sm md:text-base">{{ $partner->name }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-mono font-bold block truncate max-w-[200px] md:max-w-xs" title="{{ $partner->logo_url }}">
                            {{ $partner->logo_url }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
